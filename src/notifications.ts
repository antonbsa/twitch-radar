...
export async function upsertSubscription(
  ...
  const subscriptions =
    await readCollection<PushSubscriptionRecord>(subscriptionsFile)
...
export async function createNotification(
  ...
  const subscription = subscriptions.find(
    (record) => record.id === subscriptionId,
  )
  ...
  const scheduledFor = new Date(
    createdAt.getTime() + channel.delaySeconds * 1000,
  )
  ...
  const notifications =
    await readCollection<NotificationRecord>(notificationsFile)
...
export async function listNotifications(
  ...
  return notifications
    ...
    .sort((a, b) => {
      ...
      return (
        Date.parse(b.triggeredAt ?? b.scheduledFor) -
        Date.parse(a.triggeredAt ?? a.scheduledFor)
      )
    })
}
...
function scheduleNotification(notification: NotificationRecord): void {
  ...
  const delayMs = Math.max(
    0,
    Date.parse(notification.scheduledFor) - Date.now(),
  )
  const timer = setTimeout(() => {
    triggerNotification(notification.id).catch(() => {})
  }, delayMs)
...
async function triggerNotification(notificationId: string): Promise<void> {
  ...
  const subscription = subscriptions.find(
    (record) => record.id === notification.subscriptionId,
  )
