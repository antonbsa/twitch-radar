...
async function checkPushState() {
  if (!("serviceWorker" in navigator) || !("PushManager" in window) || !("Notification" in window)) {
    setPushState(false, "Unsupported", "Push notifications are not available in this browser.");
    return;
  }
...
async function enableNotifications() {
  ...
  try {
    ...
    setPushState(false, "Error", error instanceof Error ? error.message : "Unable to enable notifications.");
  } finally {
    renderControls();
  }
}
...
async function scheduleNotification() {
  ...
  try {
    ...
    setPushState(state.pushReady, state.pushReady ? "Ready" : "Error", error instanceof Error ? error.message : "Unable to schedule notification.");
  } finally {
    renderControls();
  }
}
...
function renderControls() {
  const notificationDenied = "Notification" in window && Notification.permission === "denied";
  elements.enableButton.disabled = state.pushReady || notificationDenied;
  elements.scheduleButton.disabled = !state.pushReady || !state.subscriptionId || !state.selectedChannelId;
}

function renderNotificationLists() {
  const waiting = state.notifications.filter((notification) => notification.status === "waiting");
  const triggered = state.notifications.filter((notification) => notification.status !== "waiting");
...
function renderList(target, notifications, section) {
  ...
  if (notifications.length === 0) {
    ...
    empty.textContent = section === "waiting" ? "No waiting notifications" : "No triggered notifications";
    target.append(empty);
    return;
  }
  ...
  for (const notification of notifications) {
    ...
    item.className = `notification-card ${section === "triggered" ? "old" : ""} ${isFailed ? "failed" : ""}`.trim();
...
function setPushState(pushReady, label, message) {
  ...
  elements.statusPill.classList.toggle("blocked", label === "Blocked" || label === "Error" || label === "Unsupported");
