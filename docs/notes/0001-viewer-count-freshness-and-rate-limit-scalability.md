...
- Not reduce total requests made to Twitch — it only moves which bucket absorbs the cost (shared app bucket vs. per-user bucket). It is a rate-limit _distribution_ change, not a request-count reduction.
