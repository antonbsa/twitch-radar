...
**`GET /api/notifications/snoozes`**, authenticated, returns the current user's `pending` reminders. The channel detail modal reads this to show the snooze button already disabled for a broadcaster/category that already has one scheduled — reopening the modal (or reloading the page) must reflect the server-side idempotency guarantee above, not just this session's own mutation result, which would otherwise reset and let the button look re-clickable even though a repeated request still no-ops.
