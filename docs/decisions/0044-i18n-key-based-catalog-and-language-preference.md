...
- A user's language preference change is immediate in the current tab (optimistic local catalog swap) and applies to all *subsequently sent* push notifications once the `PATCH` lands; a notification already in flight when the change happens uses whatever `lang` was resolved at send time.
